...
describe('QuickActionFab - researcher pill navigation', () => {
  ...
  it('navigates to /research/register?page=title when researcher pill clicked', async () => {
    render(<QuickActionFab />, { wrapper });

    // Open the speed dial
    const toggle = screen.getByRole('button', { name: /open menu/i });
    fireEvent.click(toggle);

    // Click the Register Survey pill
    const pill = await screen.findByText('Register Survey');
    fireEvent.click(pill);

    expect(mockPush).toHaveBeenCalledWith('/research/register?page=title');
  });
});
